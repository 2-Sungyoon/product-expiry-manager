...
### 1. 핵심 문제 
* **데이터의 불투명성:** POS 시스템은 판매된 상품의 바코드만 인식할 뿐, "어떤 유통기한의 상품이 판매되었는지"는 알 수 없습니다.
...
# 위 응답에서 나온 "_id" 값을 복사하여 아래 <PRODUCT_ID> 자리에 넣어주세요
...
* **트랜잭션 고도화:** 현재는 로컬 개발 환경을 고려하여 비관적 락 개념의 순차 처리를 적용했으나, 배포 환경에서는 `startSession()`을 통한 완벽한 ACID 트랜잭션으로 전환 가능합니다.
